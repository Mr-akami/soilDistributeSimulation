...
class CNNQNetwork(nn.Module):
    def __init__(self, state_shape, n_action):
        ...
        # 入力を画像ではなく離散値を取るので、Conv2DをLinearに変更
        ...
        self.fc_state = nn.Sequential(
            nn.Linear(512, 512), # conv_layersから出力されたfeatのshapeに合わせる
            nn.ReLU(),
            nn.Linear(512, 1)
        )# 1x3136 and 9088x512)
...
net = CNNQNetwork(324, n_action=env.action_space.n).to(device)
...
target_net = CNNQNetwork(324, n_action=env.action_space.n).to(device)
...
for episode in range(n_episodes):
    ...
    print('Episode: {},  Step: {},  Reward: {}, Time:{}'.format(episode + 1, step + 1, total_reward, time.time() - start))
